...
class SharedNumpyArray:
    ...
    def __init__(self, queue):
        self._queue = queue
        self._array = np.zeros(ARRAY_SAMPLES, dtype=NP_TYPE)
        self._array_shm = shm.SharedMemory(create=True, size=self._array.nbytes)
        self._queue.put(self._array_shm.name)
        self._samples = 0

    def put(self, array):
        np.append(self._array_shm, array)
        self._samples += len(array)
        self._queue.put(self._samples)
