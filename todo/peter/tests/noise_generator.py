...
def process(sample_start, samples, dt_s, sine_amp_V_rms = 0.1, noise_density_V_sqrtHz = 0.1):
    ...
    sine_freq_Hz = 0.00001 # on a E12 point, not equal to fs so we can see errors, just in case
...
x = process(sample_start=0, samples=10000, dt_s=dt_s, sine_amp_V_rms = 0.1, noise_density_V_sqrtHz = 0.001)
...
if True:
    fig, ax = plt.subplots()
    ax.plot(x)
    ax.set(xlabel='sample', ylabel='V', title='Furz')
    ax.grid()
    plt.show()
