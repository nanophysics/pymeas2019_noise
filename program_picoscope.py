...
class PicoScope:
  ...
  def acquire(self, config):
    ...
    self.scope.set_trigger('A', -1000.0, direction='below', timeout=0.2)  # use Channel A as the trigger source at 1V, wait forever for a trigger event
    self.scope.run_streaming(auto_stop=True)  # start streaming mode
