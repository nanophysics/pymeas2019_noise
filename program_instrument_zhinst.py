...
class Instrument:
  ...
  def acquire(self, configStep, stream_output, handlerCtrlC):
    ...
    while n < num_scope_samples:
      ...
      print(f'Poll #{poll_count} returned {num_blocks_poll} blocks of streamed scope data. '
        f'blocks processed {num_blocks} samples acquired {n}.', sep='', end='\r')
      for b, block in enumerate(data[stream_nodepath]):
        ...
        assert block['datatransfermode'] == 3, \
          'The blocks datatransfermode states the block does not contain scope streaming data.'
        ...
        scope_samples_value[n:n + num_samples_block] = \
          block['channeloffset'][SCOPE_INPUT_SELECT] \
          + block['channelscaling'][SCOPE_INPUT_SELECT]*block['wave'][:num_samples_block, SCOPE_INPUT_SELECT]
        
        adu_values = block['channeloffset'][SCOPE_INPUT_SELECT] \
          + block['channelscaling'][SCOPE_INPUT_SELECT]*block['wave'][:num_samples_block, SCOPE_INPUT_SELECT]
    ...
    if min_ts_delta != expected_ts_delta:
      ...
      self.warning(f'Scope channel {SCOPE_INPUT_SELECT} timestamps have a min_diff {min_ts_delta} (first discrepancy at pos: {index[0]}). '
              f'Expected {expected_ts_delta}.')
      ts_delta_mismatch = True
    if max_ts_delta != expected_ts_delta:
      index = np.where(diff_timestamps == max_ts_delta)[0]
      self.warning(f'Scope channel {SCOPE_INPUT_SELECT} timestamps have a max_diff {max_ts_delta} (first discrepenacy at pos: {index[0]}). '
              f'Expected {expected_ts_delta}.')
      ts_delta_mismatch = True
