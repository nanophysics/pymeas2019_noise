...
def main()->None:
    directory_measurement_actual = DIRECTORY_OF_THIS_FILE.parent.parent/DIRECTORY_MEASUREMENT_ACTUAL
    ...
    for target in TARGETS:
        filename_zip = directory_artifacts / FILENAME_MEASUREMENT_ACTUAL_ZIP.replace("<TARGET>",target.label)
        with zipfile.ZipFile(filename_zip, 'w') as zipf:
            for command in COMMANDS:
                content = target.content.replace("<COMMAND>", command)
                filename = f"{DIRECTORY_MEASUREMENT_ACTUAL}/{command}{target.filename_suffix}"
